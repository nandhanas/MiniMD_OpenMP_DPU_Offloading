...
class ForceEAM : Force
{
  public:
    ...
    virtual ~ForceEAM();
    virtual void compute(Atom &atom, Neighbor &neighbor, Comm &comm, int me, int pipeline);
...
void ForceEAM::compute(Atom &atom, Neighbor &neighbor, Comm &comm, int me, int pipeline)
{
...
# 518 "force_eam.cpp"
  ...
  MPI_Bcast(&flag, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), 0, hosts_communicator);
  ...
  MPI_Bcast(&file->mass, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);
  MPI_Bcast(&file->nrho, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), 0, hosts_communicator);
  MPI_Bcast(&file->drho, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);
  MPI_Bcast(&file->nr, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), 0, hosts_communicator);
  MPI_Bcast(&file->dr, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);
  MPI_Bcast(&file->cut, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);
  mass = file->mass;
  ...
  if(sizeof(MMD_float) == 4)
    MPI_Bcast(file->frho, file->nrho, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_float)))), 0, hosts_communicator);
  else
    MPI_Bcast(file->frho, file->nrho, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);

  if(me == 0) grab(fptr, file->nr, file->zr);

  if(sizeof(MMD_float) == 4)
    MPI_Bcast(file->zr, file->nr, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_float)))), 0, hosts_communicator);
  else
    MPI_Bcast(file->zr, file->nr, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);

  if(me == 0) grab(fptr, file->nr, file->rhor);

  if(sizeof(MMD_float) == 4)
    MPI_Bcast(file->rhor, file->nr, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_float)))), 0, hosts_communicator);
  else
    MPI_Bcast(file->rhor, file->nr, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), 0, hosts_communicator);
  ...
  for(iswap = 0; iswap < comm.nswap; iswap++) {
    ...
    if(comm.sendproc[iswap] != me) {
      ...
      MPI_Sendrecv(comm.buf_send, comm.comm_send_size[iswap], (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_float)))), comm.sendproc[iswap], 0,
                   ...
                   hosts_communicator, (static_cast<MPI_Status *> (0)));
      buf = comm.buf_recv;
