...
void Atom::growarray()
{
  ...
  sorted_index = realloc_1d_int_array(sorted_index, nmax, 0);
...
# 98 "atom.cpp"
    ...
    if(isHost){
        ...
        MPI_Win_attach(win_x, x, sizeof(MMD_float)*nmax*3);
        MPI_Win_attach(win_f, f, sizeof(MMD_float)*nmax*3);
...
int Atom::unpack_exchange_bf(int i, MMD_float* buf)
{
  if(i == nmax) growarray();
...
void Atom::sort_bf()
{
  if(copy_size<nmax) {
     ...
     f_copy = (MMD_float*) create_2d_MMD_float_array(nmax, 3);
      copy_size = nmax;
  }
...
# 513 "atom.cpp"
  ...
  for(int i = 0; i<nlocal; i++) {
   ...
   new_f[i*3 +0] = old_f[sorted_index[i]*3 +0];
   new_f[i*3 +1] = old_f[sorted_index[i]*3 +1];
   new_f[i*3 +2] = old_f[sorted_index[i]*3 +2];
 }
