...
void read_lammps_header(Atom &atom)
{
  ...
  const char* section_keywords[3] =
  {"Atoms", "Velocities", "Masses"};



  char* eof = fgets(line, 255, fp);
...
# 220 "setup.cpp"
  ...
  MMD_float* x = atom.create_2d_MMD_float_array(atom.natoms, 3);
  MMD_float* v = atom.create_2d_MMD_float_array(atom.natoms, 3);
  ...
  for(int i = 0; i < atom.natoms; i++) {
    if(x[i * 3 + 0] >= atom.box.xlo && x[i * 3 + 0] < atom.box.xhi &&
        x[i * 3 + 1] >= atom.box.ylo && x[i * 3 + 1] < atom.box.yhi &&
        x[i * 3 + 2] >= atom.box.zlo && x[i * 3 + 2] < atom.box.zhi)
      atom.addatom(x[i * 3 + 0], x[i * 3 + 1], x[i * 3 + 2], v[i * 3 + 0], v[i * 3 + 1], v[i * 3 + 2]);
  }
...
int create_atoms(Atom &atom, int nx, int ny, int nz, double rho)
{
  ...
  ilo = ((ilo) > (0) ? (ilo) : (0));
  ihi = ((ihi) < (2 * nx - 1) ? (ihi) : (2 * nx - 1));
  jlo = ((jlo) > (0) ? (jlo) : (0));
  jhi = ((jhi) < (2 * ny - 1) ? (jhi) : (2 * ny - 1));
  klo = ((klo) > (0) ? (klo) : (0));
  khi = ((khi) < (2 * nz - 1) ? (khi) : (2 * nz - 1));
# 348 "setup.cpp"
  ...
  int iflagall;
  MPI_Allreduce(&iflag, &iflagall, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), (static_cast<MPI_Op> (static_cast<void *> (&(ompi_mpi_op_max)))), hosts_communicator);
...
void create_velocity(double t_request, Atom &atom, Thermo &thermo)
{
  ...
  double tmp;
  MPI_Allreduce(&vxtot, &tmp, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), (static_cast<MPI_Op> (static_cast<void *> (&(ompi_mpi_op_sum)))), hosts_communicator);
  vxtot = tmp / atom.natoms;
  MPI_Allreduce(&vytot, &tmp, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), (static_cast<MPI_Op> (static_cast<void *> (&(ompi_mpi_op_sum)))), hosts_communicator);
  vytot = tmp / atom.natoms;
  MPI_Allreduce(&vztot, &tmp, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_double)))), (static_cast<MPI_Op> (static_cast<void *> (&(ompi_mpi_op_sum)))), hosts_communicator);
