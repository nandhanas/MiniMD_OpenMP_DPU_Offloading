...
# 1 "openmp.h" 1
# 40 "openmp.h"
# 1 "/usr/lib/gcc/x86_64-redhat-linux/8/include/omp.h" 1 3 4
# 35 "/usr/lib/gcc/x86_64-redhat-linux/8/include/omp.h" 3 4
...
{
  maxsend = 1000;
  buf_send = (MMD_float*) malloc((maxsend + 1000) * sizeof(MMD_float));
  maxrecv = 1000;
...
int Comm::setup(MMD_float cutneigh, Atom &atom)
{
  ...
  MPI_Comm_rank(hosts_communicator, &me);
  MPI_Comm_size(hosts_communicator, &nprocs);
  ...
  MPI_Cart_create(hosts_communicator, 3, procgrid, periods, reorder, &cartesian);
...
# 516 "comm.cpp"

    {
      ...
      MPI_Sendrecv(&nsend, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), procneigh[idim][0], 0,
                   &nrecv1, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), procneigh[idim][1], 0,
                   hosts_communicator, (static_cast<MPI_Status *> (0)));
      nrecv = nrecv1;

      if(procgrid[idim] > 2) {
        MPI_Sendrecv(&nsend, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), procneigh[idim][1], 0,
                     &nrecv2, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), procneigh[idim][0], 0,
                     hosts_communicator, (static_cast<MPI_Status *> (0)));
        nrecv += nrecv2;
      }
  ...
  for(idim = 0; idim < 3; idim++) {
    ...
    for(int ineed = 0; ineed < 2 * need[idim]; ineed += 1) {
      if(ineed < procgrid[idim] - 1) {
        MPI_Sendrecv(&nsend, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), sendproc_exc[iswap], 0,
                     &nrecv, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), recvproc_exc[iswap], 0,
                     hosts_communicator, (static_cast<MPI_Status *> (0)));
...
# 821 "comm.cpp"

      {
        ...
        if(sendproc[iswap] != me) {
          MPI_Sendrecv(&nsend, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), sendproc[iswap], 0,
                       &nrecv, 1, (static_cast<MPI_Datatype> (static_cast<void *> (&(ompi_mpi_int)))), recvproc[iswap], 0,
                       hosts_communicator, (static_cast<MPI_Status *> (0)));
...
void Comm::growsend(int n)
{
  ...
  buf_send = (MMD_float*) realloc(buf_send, (maxsend + 100) * sizeof(MMD_float));
}
...
void Comm::exchange_bf(Atom &atom)
{
...
# 1042 "comm.cpp"
    ...
    for(int k = 0; k < nsend; k++) {
      ...
      if(exc_sendlist_thread[tid][k] < nlocal - total_nsend) {
        ...
        atom.copy_bf(j++, exc_sendlist_thread[tid][k]);
      }
    }
...
# 1139 "comm.cpp"
    ...
    for(int i = 0; i < nrecv_atoms; i++) {
      ...
      if(value >= lo && value < hi)
        atom.unpack_exchange_bf(copyinpos++, &buf_recv[i * 6]);
    }
