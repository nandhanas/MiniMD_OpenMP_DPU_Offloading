...
void ForceLJ::compute(Atom &atom, Neighbor &neighbor, Comm &comm, int me, int pipeline)
{
  eng_vdwl = 0;
  virial = 0;
  ...
  if(evflag) {
    ...
    } else return compute_fullneigh<1>(atom, neighbor, me, pipeline);
  } else {
    if(use_oldcompute)
    ...
    } else return compute_fullneigh<0>(atom, neighbor, me, pipeline);
...
# 366 "force_lj.cpp"
template<int EVFLAG>
void ForceLJ::compute_fullneigh(Atom &atom, Neighbor &neighbor, int me, int pipeline)
{
