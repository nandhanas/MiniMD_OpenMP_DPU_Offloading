...
void Integrate::initialIntegrate(int ishost, int n)
{
  double c = 1;
  if(!isHost && n>0){
    c = 2;
  ...
  for(MMD_int i = 0; i < nlocal; i++) {
    v[i * PAD + 0] += c * dtforce * f[i * PAD + 0];
    v[i * PAD + 1] += c * dtforce * f[i * PAD + 1];
    v[i * PAD + 2] += c * dtforce * f[i * PAD + 2];
                    ...
                    Comm &comm, Thermo &thermo, Timer &timer)
{
  ...
  #pragma omp parallel private(i,n)
  {
    ...
    for(n = 0; n < ntimes; n++) {
      ...
      if(!isHost){
        if(n > 0){
          if((n) % neighbor.every ==0){
            comm_x_to_bf(atom, neighbor, comm, n);
          }
          MPI_Recv(atom.f, atom.nlocal*PAD,MPI_DOUBLE, 0, 0, nic_host_communicator,MPI_STATUS_IGNORE);
        }
        x = atom.x;
        v = atom.v;
        f = atom.f;
        ...
        initialIntegrate(isHost, n);
        MPI_Send(atom.x, atom.nlocal*PAD,MPI_DOUBLE, 0, 0, nic_host_communicator);
        MPI_Send(atom.v, atom.nlocal*PAD,MPI_DOUBLE, 0, 0, nic_host_communicator);
      }
      ...
      timer.stamp();
      if(isHost){
        MPI_Recv(atom.x, atom.nlocal*PAD,MPI_DOUBLE, 1, 0, nic_host_communicator,MPI_STATUS_IGNORE);
        MPI_Recv(atom.v, atom.nlocal*PAD,MPI_DOUBLE, 1, 0, nic_host_communicator,MPI_STATUS_IGNORE);
        x = atom.x;
        v = atom.v;
        f = atom.f;
        xold = atom.xold;
      ...
      if(n!=(ntimes-1)) MPI_Send(atom.f, atom.nlocal*PAD,MPI_DOUBLE, 1, 0, nic_host_communicator);
